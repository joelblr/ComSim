...
//	public static final String bg_white =	"\u001B[47m";
	...
	protected static String colourMsg(String[] txtFormat) {
		...
		for (String fmt : txtFormat[len-1].trim().split(" ")) {
			if (i == 1)
				keys.add(fmt);
			else
				seps.add(fmt);
		...
		if (seps.isEmpty()) {
//			String ans = msgColour(keys.get(0), txtFormat[0]);
////			System.out.print(ans + " 00000");
//			return (len == 3) ? ans + " " + txtFormat[1] : ans;
			return msgColour(keys.get(0), txtFormat[0]);
		}

		int idx = 0;
		String newTxt = "";
		for (i = 0; i < txtFormat[0].length(); i++) {
			String ch = txtFormat[0].charAt(i)+"";
...
//		if (len == 3)
//		System.out.println(newTxt + " " + txtFormat[1]);
//		else 
//		System.out.println(newTxt);
//		return (len == 3) ? newTxt + " " + txtFormat[1] : newTxt;
		return newTxt;
	}
